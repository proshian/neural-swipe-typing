...
class LitNeuroswipeModel(LightningModule):
    ...
    def forward(self, x, y, t, grid_names, decoder_in, swipe_pad_mask, word_pad_mask):
        return self.model.forward(x, y, t, grid_names, decoder_in, swipe_pad_mask, word_pad_mask)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # batch_x is (x, y, t, grid_names, decoder_in, swipe_pad_mask, word_pad_mask)
        x, y, t, grid_names, decoder_in, swipe_pad_mask, word_pad_mask = batch_x

        pred = self.forward(x, y, t, grid_names, decoder_in, swipe_pad_mask, word_pad_mask)

        loss = self.criterion(pred, batch_y)

        argmax_pred = torch.argmax(pred, dim=2)
        wl_acccuracy = get_word_level_accuracy(
            argmax_pred.T, batch_y.T, pad_token = self.word_pad_idx, mask = word_pad_mask)
